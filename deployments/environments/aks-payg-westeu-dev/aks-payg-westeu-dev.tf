terraform {
  backend "azurerm" {
    ...
    key                  = "aks-payg-westeu-dev.tfstate"
  }
}
...
module "aks_cluster" {
  ...
  cluster_name              = "aks-payg-westeu-dev"
  cluster_vm_size           = "Standard_D8s_v3" # Expensive fast machine (don't forget to destroy after use)
  cluster_vm_disk_size      = 128               # Max size of it's Ephemeral disk
  cluster_vm_min_node_count = 1
  cluster_vm_max_node_count = 1
  cluster_vm_max_pods_count = 90
